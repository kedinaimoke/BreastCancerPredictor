...
def add_predictions(input_data):
	...
	st.write(f"Probability of being BENIGN: <span class='prob benign'>{model.predict_proba(input_array_scaled)[0][0]}</span>"
                , unsafe_allow_html=True)
	st.write(f"Probability of being MALIGNANT: <span class='prob malignant'>{model.predict_proba(input_array_scaled)[0][1]}</span>"
                , unsafe_allow_html=True)
	
	st.write("DISCLAIMER: This app can assist medical professionals in making a diagnosis"
...
with st.container():
    ...
    st.write("This app uses a machine learning model to determine if a breast mass is BENIGN or MALIGNANT based on"
    " the measurements it receives from your cytosis lab, so do well to connect this app to your cytology lab to "
    "help diagnose breast cancer from your tissue sample. You can adjust measurements by hand using the sliders "
    "in the left sidebar.")
